    ...
    <div class="content-header row align-items-center m-0">
        <nav aria-label="breadcrumb" class="col-sm-4 order-sm-last mb-3 mb-sm-0 p-0 ">
            <ol class="breadcrumb d-inline-flex font-weight-600 fs-13 bg-white mb-0 float-sm-right">
                ...
                <li class="breadcrumb-item active">{{__('words.agent_report')}}</li>
            </ol>
        </nav>
        <div class="col-sm-8 header-title p-0">
            <div class="media">
                ...
                <div class="media-body">
                    <h1 class="font-weight-bold">{{__('words.agent_report')}}</h1>
                    <small>{{__('words.agent')}} - {{ $agent->username }}</small>
                </div>
            </div>
        </div>
    </div> 
                ...
                <div class="card card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                @foreach ($games as $item)
                                    @php
                                        ...
                                        $sale = $agent->game_records()->whereBetween('bet_date', $period)->where('game_id', $item->id)->sum('win_lose_amount');
                            ...
                            <tfoot>
                                <tr>
                                    <th colspan="3">Total :  {{$total_sale}} * {{$rate}}% = {{$total_commission}}</th>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                @for ($dt=$start; $dt <= $end; $dt->addDay())
                                    @php
                                        $key = $dt->format('Y-m-d');
                                        $mod = $agent->game_records()->whereDate('bet_date', $key);
                                        if($game_id != '') {
